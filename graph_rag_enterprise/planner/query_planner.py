from llm.intent_classifier import IntentClassifier
import os

class QueryPlanner:

    def __init__(self):
        # Try to load ML model, fallback to rule-based if not available
        model_path = os.path.join(os.path.dirname(__file__), '..', 'models', 'intent_classifier')
        if os.path.exists(model_path):
            try:
                self.classifier = IntentClassifier(model_path)
                self.use_ml = True
                print("✅ Intent Classifier loaded - using ML for intent detection")
            except Exception as e:
                print(f"⚠️ Failed to load Intent Classifier: {e}")
                self.classifier = None
                self.use_ml = False
        else:
            self.classifier = None
            self.use_ml = False
            print("ℹ️ Intent Classifier not found - using rule-based planning")

    def plan(self, query, mapped):

        q = query.lower()

        sizes = [
            r["value"] for r in mapped
            if r.get("column") == "size"
        ]

        # Rule-based plan remains primary for strong signals
        rule_plan = self._rule_based_plan(q, sizes, mapped)
        if rule_plan["type"] != "NO_MATCH":
            return rule_plan

        # If rule-based cannot decide, use ML intent classification as a fallback
        if self.use_ml:
            ml_result = self.classifier.predict(query)
            intent = ml_result['intent']
            confidence = ml_result['confidence']

            print(f"🤖 ML Intent: {intent} (confidence: {confidence:.2f})")
            if confidence > 0.7:
                return self._ml_to_plan(intent, sizes, mapped)

        return rule_plan

    def _rule_based_plan(self, q: str, sizes: list, mapped: list) -> dict:
        print("📋 Using rule-based planning")

        columns = [r.get("column") for r in mapped]

        if not sizes and any(phrase in q for phrase in ["mẫu này", "mau nay", "cái này", "cai nay"]):
            return {
                "type": "NO_MATCH",
                "reason": "Referent missing size or model"
            }

        if "tai_trong_lon_nhat" in columns:
            if sizes:
                if not any(phrase in q for phrase in ["cao nhất", "cao nhat", "tốt nhất", "max", "lớn nhất", "nhiều nhất"]):
                    return {
                        "type": "LOAD",
                        "sizes": sizes
                    }
                return {
                    "type": "MAX_LOAD",
                    "sizes": sizes
                }
            if any(phrase in q for phrase in ["cao nhất", "cao nhat", "tốt nhất", "max", "lớn nhất", "nhiều nhất"]):
                return {
                    "type": "MAX_LOAD",
                    "sizes": sizes
                }
            return {
                "type": "NO_MATCH",
                "reason": "Need a specific size or a stronger load signal"
            }

        if "toc_do_toi_da" in columns:
            if sizes:
                if not any(phrase in q for phrase in ["cao nhất", "cao nhat", "tốt nhất", "max", "lớn nhất", "nhiều nhất"]):
                    return {
                        "type": "SPEED",
                        "sizes": sizes
                    }
                return {
                    "type": "MAX_SPEED",
                    "sizes": sizes
                }
            if any(phrase in q for phrase in ["cao nhất", "cao nhat", "tốt nhất", "max", "lớn nhất", "nhiều nhất"]):
                return {
                    "type": "MAX_SPEED",
                    "sizes": sizes
                }
            return {
                "type": "NO_MATCH",
                "reason": "Need a specific size or a stronger speed signal"
            }

        if "gia_ban_co_vat" in columns:
            if sizes and "cao nhất" not in q:
                return {
                    "type": "PRICE",
                    "sizes": sizes
                }
            if any(phrase in q for phrase in ["cao nhất", "cao nhat", "tốt nhất", "max", "lớn nhất", "nhiều nhất"]):
                return {
                    "type": "MAX_PRICE",
                    "sizes": sizes
                }
            return {
                "type": "NO_MATCH",
                "reason": "Need a specific size or a stronger price signal"
            }

        if ("cao nhất" in q and "tải" in q) or "chịu tải" in q:
            return {
                "type": "MAX_LOAD",
                "sizes": sizes
            }

        if "giá" in q and sizes and "cao nhất" not in q:
            return {"type": "PRICE", "sizes": sizes}

        if "công ty" in q:
            return {"type": "MULTI_HOP", "path": ["Tire", "Company"], "sizes": sizes}

        if "tiêu chuẩn" in q:
            return {"type": "MULTI_HOP", "path": ["Tire", "QualityStandard"], "sizes": sizes}

        if "van" in q:
            return {"type": "MULTI_HOP", "path": ["Tire", "Tube", "Van"], "sizes": sizes}

        if "so sánh" in q:
            if sizes:
                return {"type": "COMPARE", "sizes": sizes}
            return {
                "type": "NO_MATCH",
                "reason": "Need specific tires or sizes to compare"
            }

        if "giá" in q and "cao nhất" in q:
            return {
                "type": "MAX_PRICE",
                "sizes": sizes
            }

        if sizes:
            return {"type": "SINGLE", "sizes": sizes}

        return {
            "type": "NO_MATCH",
            "reason": "No signal detected"
        }

    def _ml_to_plan(self, intent: str, sizes: list, mapped: list) -> dict:
        """Convert ML intent to plan format"""

        if intent == "SINGLE":
            return {"type": "SINGLE", "sizes": sizes}

        elif intent == "COMPARE":
            return {"type": "COMPARE", "sizes": sizes}

        elif intent == "MAX_SPEED":
            return {"type": "MAX_SPEED", "sizes": sizes}

        elif intent == "MAX_LOAD":
            return {"type": "MAX_LOAD", "sizes": sizes}

        elif intent == "PRICE":
            return {"type": "PRICE", "sizes": sizes}

        elif intent == "MULTI_HOP":
            # Determine path based on query content
            query_lower = " ".join([r.get("original_query", "") for r in mapped if r.get("original_query")])
            if "công ty" in query_lower:
                return {"type": "MULTI_HOP", "path": ["Tire", "Company"], "sizes": sizes}
            elif "tiêu chuẩn" in query_lower:
                return {"type": "MULTI_HOP", "path": ["Tire", "QualityStandard"], "sizes": sizes}
            elif "van" in query_lower:
                return {"type": "MULTI_HOP", "path": ["Tire", "Tube", "Van"], "sizes": sizes}
            else:
                return {"type": "MULTI_HOP", "path": ["Tire", "Company"], "sizes": sizes}

        elif intent == "NO_MATCH":
            return {"type": "NO_MATCH", "reason": "Ambiguous query - need more context"}

        else:
            # Fallback to rule-based for unknown intents
            return self.plan(" ".join([r.get("original_query", "") for r in mapped if r.get("original_query")]), mapped)